...
# Ensure pattern resource scripts are loaded (Godot should handle via class_name,
# but we force references for safety):
# ensure subclass scripts loaded
const _REF_GRID = preload("res://addons/csg_toolkit/scripts/patterns/grid_pattern.gd")
...
@export var pattern: CSGPattern:
	...
	set(value):
		...
		if value != null and value.get_class() == "CSGPattern":
			push_warning("Cannot assign base CSGPattern directly. Please use a concrete pattern.")
			return
		# Disconnect old
...
# Per-axis rotation variance in degrees (0 = full 0..360 random for that axis;
# >0 jitters around original)
var rotation_variance_x_deg: float:
	get:
		return _rotation_variance_x_deg
	set(value):
...
var rotation_variance_y_deg: float:
	get:
		return _rotation_variance_y_deg
	set(value):
...
var rotation_variance_z_deg: float:
	get:
		return _rotation_variance_z_deg
	set(value):
...
var scale_variance_z: float:
	get:
		return _scale_variance_z
	set(value):
		_scale_variance_z = clamp(value, 0.0, 1.0)
		if _randomize_scale and _randomize_scale_z:
			_mark_dirty()

# Per-axis scale randomization toggles (optional – if none enabled acts as
# uniform variance on all axes)
var randomize_scale_x: bool:
	get:
		return _randomize_scale_x
	set(value):
...
func repeat_template() -> void:
	...
	var template_size: Vector3 = _get_template_size(template_node)
	var ctx_cap: Dictionary = {
	...
	if estimate > MAX_INSTANCES:
		push_warning(
			(
				"CSGRepeater3D: Estimated count %s exceeds cap %s. Aborting generation."
				% [estimate, MAX_INSTANCES]
			)
		)
		_generation_in_progress = false
		return
...
func _get_template_size(template_node: Node) -> Vector3:
	if template_node == null or not (template_node is Node3D):
		return Vector3.ONE
	var aabb: AABB = _get_combined_aabb(template_node)
...
func _apply_variations(instance: Node3D) -> void:
	if _randomize_rotation:
		var final_rot: Vector3 = instance.rotation
		if _randomize_rot_x:
			if _rotation_variance_x_deg > 0.0:
				final_rot.x += rng.randf_range(
					-deg_to_rad(_rotation_variance_x_deg), deg_to_rad(_rotation_variance_x_deg)
				)
			else:
				final_rot.x = rng.randf() * TAU
		if _randomize_rot_y:
			if _rotation_variance_y_deg > 0.0:
				final_rot.y += rng.randf_range(
					-deg_to_rad(_rotation_variance_y_deg), deg_to_rad(_rotation_variance_y_deg)
				)
			else:
				final_rot.y = rng.randf() * TAU
		if _randomize_rot_z:
			if _rotation_variance_z_deg > 0.0:
				final_rot.z += rng.randf_range(
					-deg_to_rad(_rotation_variance_z_deg), deg_to_rad(_rotation_variance_z_deg)
				)
			else:
	...
	if _randomize_scale:
		...
		if use_axes:
			...
			if _randomize_scale_x:
				var vx: float = _scale_variance_x if _scale_variance_x > 0.0 else _scale_variance
				sx *= max(0.1, 1.0 + rng.randf_range(-vx, vx))
			if _randomize_scale_y:
				var vy: float = _scale_variance_y if _scale_variance_y > 0.0 else _scale_variance
				sy *= max(0.1, 1.0 + rng.randf_range(-vy, vy))
			if _randomize_scale_z:
				var vz: float = _scale_variance_z if _scale_variance_z > 0.0 else _scale_variance
				sz *= max(0.1, 1.0 + rng.randf_range(-vz, vz))
			instance.scale = Vector3(sx, sy, sz)
		else:
			var scale_factor: float = max(
				0.1, 1.0 + rng.randf_range(-_scale_variance, _scale_variance)
			)
...
func _get_property_list() -> Array:
	...
	props.append({"name": "Variation Options", "type": TYPE_NIL, "usage": PROPERTY_USAGE_GROUP})
	# Rotation subgroup under Variation Options
	props.append(
		{"name": "Rotation Randomization", "type": TYPE_NIL, "usage": PROPERTY_USAGE_SUBGROUP}
	)
	...
	props.append({"name": "Scale Variation", "type": TYPE_NIL, "usage": PROPERTY_USAGE_SUBGROUP})
	props.append(
		{
...
func _prop_bool(name: String) -> Dictionary:
	return {
		"name": name, "type": TYPE_BOOL, "usage": PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_EDITOR
	}


func _prop_float_deg(name: String, _value: float) -> Dictionary:
	return {
		"name": name,
		"type": TYPE_FLOAT,
		"hint": PROPERTY_HINT_RANGE,
