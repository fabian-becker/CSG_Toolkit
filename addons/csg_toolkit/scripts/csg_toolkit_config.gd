@tool
class_name CsgTkConfig extends Node
...
var default_behavior: CSGBehavior = CSGBehavior.SIBLING:
	get:
		return _get_setting(SETTING_DEFAULT_BEHAVIOR, DEFAULT_DEFAULT_BEHAVIOR)
	set(value):
		_set_setting(SETTING_DEFAULT_BEHAVIOR, value)

## Key to hold for primary action (e.g., adding CSG nodes)
var action_key: Key = KEY_SHIFT:
	get:
		return _get_setting(SETTING_ACTION_KEY, DEFAULT_ACTION_KEY)
	set(value):
		_set_setting(SETTING_ACTION_KEY, value)

## Key to hold for secondary action (e.g., alternative CSG operations)
var secondary_action_key: Key = KEY_ALT:
	get:
		return _get_setting(SETTING_SECONDARY_ACTION_KEY, DEFAULT_SECONDARY_ACTION_KEY)
	set(value):
		_set_setting(SETTING_SECONDARY_ACTION_KEY, value)

## Whether to auto-hide the CSG toolkit UI when not in use
var auto_hide: bool = true:
	get:
		return _get_setting(SETTING_AUTO_HIDE, DEFAULT_AUTO_HIDE)
	set(value):
		_set_setting(SETTING_AUTO_HIDE, value)
...
func _ensure_settings_exist() -> void:
	...
	if not ProjectSettings.has_setting(SETTING_ACTION_KEY):
		...
		ProjectSettings.add_property_info(
			{"name": SETTING_ACTION_KEY, "type": TYPE_INT, "hint": PROPERTY_HINT_NONE}
		)
	...
	if not ProjectSettings.has_setting(SETTING_SECONDARY_ACTION_KEY):
		...
		ProjectSettings.set_initial_value(
			SETTING_SECONDARY_ACTION_KEY, DEFAULT_SECONDARY_ACTION_KEY
		)
		ProjectSettings.add_property_info(
			{"name": SETTING_SECONDARY_ACTION_KEY, "type": TYPE_INT, "hint": PROPERTY_HINT_NONE}
		)
	...
	if not ProjectSettings.has_setting(SETTING_AUTO_HIDE):
		...
		ProjectSettings.add_property_info({"name": SETTING_AUTO_HIDE, "type": TYPE_BOOL})
